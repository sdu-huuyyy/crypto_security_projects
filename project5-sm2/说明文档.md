...
**签名生成过程**（消息$M$，用户$ID$）：  
1. 计算$Z_A = \text{SM3}(\text{ENTL}_A \parallel \text{ID}_A \parallel a \parallel b \parallel x_G \parallel y_G \parallel x_A \parallel y_A)$  
2. $e = \text{SM3}(Z_A \parallel M)$  
3. 生成随机数$k \in [1, n-1]$  
4. 计算曲线点$(x_1, y_1) = [k]G$  
5. $r = (e + x_1) \mod n$  
6. $s = (1+d_A)^{-1}(k - r d_A) \mod n$  
7. 输出签名$(r, s)$  
